...
{
	double ans = 0;
	for(int i = 0, j = i+1;i < top;i++)
	{
		while(j+1 < (i+top) && greater(area(hull[i], hull[i+1], hull[j+1]), area(hull[i], hull[i+1], hull[j]))) j++;
		ans = max(ans, max(dist(hull[i], hull[j]), dist(hull[i+1], hull[j]))); //two max -- robuster
	}
	printf("%d", int(ans*ans + 0.5));
}
int main()
{
	graham();
